<template>
    <div>选择长度
        <el-select class="select" v-model="value" placeholder="选择密钥长度" @change="((value) => getval(value))">
    ...
    <div>
        ...
        <el-input class="outputprikey" v-model="privatekey" :rows="10" type="textarea"
            placeholder="请选择长度,当选择的长度为2048和4096时,可能需要等待较长时间" />
    </div>
    <div>
        <div class="text">公钥</div>
        <el-input class="outputpubkey" v-model="publickey" :rows="10" type="textarea"
            placeholder="请选择长度,当选择的长度为2048和4096时,可能需要等待较长时间" />
    </div>
